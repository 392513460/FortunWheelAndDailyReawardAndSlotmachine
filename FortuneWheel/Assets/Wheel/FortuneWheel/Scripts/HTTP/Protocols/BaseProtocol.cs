...
namespace Ucss
{
    ...
    public class BaseProtocol : MonoBehaviour
    {
        ...
        public void AddTransaction(string id, object data, object request = null, int timeOut = 0)
        {
            ...
            UCSSRequest uRequest = (UCSSRequest)request;
            if (uRequest.maxResends == 0)
            {
                uRequest.maxResends = UCSSconfig.maxResendTries;
            }
            if (uRequest.maxTimeOutTries == 0)
            {
                uRequest.maxTimeOutTries = UCSSconfig.maxTimeOutTries;
            }
        ...
        {
            UCSSRequest request = (UCSSRequest)transaction.request;
            if (!string.IsNullOrEmpty(request.transactionId))
            {
                if (request.IsTimeOutTryAllowed())
                {
            ...
            if (this._transactions.Count > 0 && (Time.realtimeSinceStartup - this._lastGarbageCheck > UCSSconfig.garbageCheckLimit))
            {
                ...
                if (toRemove.Count > 0)
                {
                    for (int i = 0; i < toRemove.Count; i++)
                    {
                        ...
                        UDebug.LogWarning("[BaseProtocol] old transaction [" + toRemove[i] + "] is removed");
                    }
                }
